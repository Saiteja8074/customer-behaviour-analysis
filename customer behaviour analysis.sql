...
select customer_id as customers,purchase_amount
from customer
...
select item_purchased as product ,round(avg(review_rating),2) as average_review_rating
from customer
...
select item_purchased as products,round(sum(case when discount_applied='yes' then 1 else 0 end)/count(*)*100,2) as discount_percent
from customer
group by item_purchased
